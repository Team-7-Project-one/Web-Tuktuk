<footer class="ftco-footer ftco-section img px-3" style="background-image: url(img/footer_bg.jpg); background-size: cover; background-position: center center;">
    <div class="overlay"></div>
    <div class="container">
    <div class="row mb-5">
        <div class="col-md">
        <div class="ftco-footer-widget mb-4">
            <h2 class="ftco-heading-2">Tuktuk Tour</h2>
            <p style="text-align: justify" class="mt-5">Tuktuk Tour is a website that offers information about  Sub-District Tuktuk and the district itself. The informations are governance, history & origins, culture, and tourism. Furthermore, this website also attracts tourism from hotels, souvenirs, and event information in the Tuktuk area.</p>
            <ul class="ftco-footer-social list-unstyled float-md-left float-lft mt-5 mx-5">
            <!-- <li class="ftco-animate rounded-circle" style="background-color: #3498DB;">
                <a href="#" target="_blank"><span class="icon-twitter"></span></a>
            </li> -->
            <li class="ftco-animate rounded-circle" style="background-color: #2600fd;">
                <a href="https://web.facebook.com/profile.php?id=100068895428353" target="_blank"><span class="icon-facebook"></span></a>
            </li>
            <li class="ftco-animate rounded-circle" style=" background: linear-gradient(to right, #cc2b5e 0%, #753a88 100%);">
                <a href="https://www.instagram.com/tuktuk_tour/" target="_blank"><span class="icon-instagram"></span></a>
            </li>
            </ul>
        </div>
        </div>
        <div class="col-md">
        <div class="ftco-footer-widget mb-4 ml-md-5">
            <h2 class="ftco-heading-2">Direct Links</h2>
            <ul class="list-unstyled">
            <li><a href="/hotel" class="py-2 d-block">Hotel</a></li>
            <li><a href="/souvenir" class="py-2 d-block">Souvenir</a></li>
            <li><a href="/event" class="py-2 d-block">Event</a></li>
            <li><a href="/aboutus" class="py-2 d-block">About Us</a></li>
            </ul>
        </div>
        </div>
        <div class="col-md">
        <div class="ftco-footer-widget mb-4">
            <h2 class="ftco-heading-2">Related Links</h2>
            <ul class="list-unstyled">
            <!-- <li><a href="https://www.del.ac.id/" target="_blank" class="py-2 d-block">Institut Teknologi Del</a></li> -->
            <li><a href="https://samosirkab.go.id/" target="_blank" class="py-2 d-block">Pemerintah Kabupaten Samosir</a></li>
            <li><a href="https://www.kemenparekraf.go.id/" target="_blank" class="py-2 d-block">Kementerian Pariwisata dan Ekonomi Kreatif - RI</a></li>
            <li><a href="https://www.indonesia.travel/id/id/home" target="_blank" class="py-2 d-block">Wonderfull Indonesia</a></li>
            </ul>
        </div>
        </div>
        <div class="col-md">
        <div class="ftco-footer-widget mb-4">
            <h2 class="ftco-heading-2">Have a Questions?</h2>
            <div class="block-23 mb-3">
            <ul>
                <li class="mt-5">
                    <a href="https://goo.gl/maps/ZqAhHioBZPDGicUH8"><span class="icon icon-map-marker"></span><span class="text">Tuktuk Siadong, Simanindo, Samosir, North Sumatera, Indonesia 22395</span></a>
                </li>
                <li>
                    <a href="https://api.whatsapp.com/send?phone=6285268615405"><span class="icon icon-phone"></span><span class="text">+6285268615405</span></a>
                </li>
                <li>
                    <a href="mailto:tuktuktour21@gmail.com"><span class="icon icon-envelope"></span><span class="text">tuktuktour21@gmail.com</span></a>
                </li>
            </ul>
            </div>
        </div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12 text-center">
        <a href="https://www.del.ac.id/"><img src="img/logoDel.jpg" width="50px" class="pb-3"></a>
        <p>
            Copyright &copy;
            <script>
            document.write(new Date().getFullYear());
            </script> All Rights Reserved | This Website Is Developed By&nbsp;<span style="color: #ffff00;">Kelompok 07 PA 1 - IT Del</span>
        </p>
        </div>
    </div>
    </div>
    <link rel="stylesheet" href="{{asset('css/style.css')}}" />
</footer>